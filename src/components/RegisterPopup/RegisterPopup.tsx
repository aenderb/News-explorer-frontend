...
function RegisterPopup({ isOpen, onClose, onRegister, onSwitchToLogin }: RegisterPopupProps) {
  ...
  const isFormValid = isEmailValid && password.length >= 6 && username.length >= 2;
  const showEmailError = email.length > 0 && !isEmailValid;
  const showPasswordError = password.length > 0 && password.length < 6;
  const showUsernameError = username.length > 0 && username.length < 2;
  ...
  return (
    <PopupWithForm title="Inscrever-se" isOpen={isOpen} onClose={onClose}>
